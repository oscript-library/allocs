...
Процедура ТестДолжен_ПроверитьФиксациюПотребленияПамятиДляМассиваСтрок() Экспорт
	...
	Если ВерсияСреды = "1" Тогда
		Ожидаем
			.Что(ВыделеноБайт / КоличествоЭлементов)
			.Больше(ОжидаемоеЗначениеВерсия1);
	ИначеЕсли ВерсияСреды = "2" Тогда
		Ожидаем
			.Что(ВыделеноБайт / КоличествоЭлементов)
			.Между(ОжидаемоеЗначениеВерсия2, ОжидаемоеЗначениеВерсия2 * Погрешность);
	Иначе
